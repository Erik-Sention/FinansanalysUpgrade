...
def get_seasonal_data(company_id, years):
    """Hämta data för säsongsanalys över flera år"""
    try:
        firebase_db = get_firebase_db()
...
def show():
    ...
    if selected_accounts:
        ...
        # SÄSONGSANALYS-SEKTION
        st.markdown("---")
        st.markdown("### 📅 Säsongsanalys")
        
        # Hämta tillgängliga år för säsongsanalys
        try:
            firebase_db = get_firebase_db()
            test_data_ref = firebase_db.get_ref("test_data")
            test_data = test_data_ref.get(firebase_db._get_token())
            
            available_years_seasonal = []
            if test_data and test_data.val():
                values_data = test_data.val().get('values', {})
                years_found = set()
                for value_id, value_data in values_data.items():
                    if value_data.get('company_id') == selected_company_id:
                        years_found.add(value_data.get('year'))
                available_years_seasonal = sorted(list(years_found))
        except Exception as e:
            st.error(f"Fel vid hämtning av år för säsongsanalys: {e}")
            available_years_seasonal = [selected_year]
        
        # Årval för säsongsanalys
        col1, col2 = st.columns([2, 1])
        
        with col1:
            selected_years_seasonal = st.multiselect(
                "Välj år för säsongsanalys",
                available_years_seasonal,
                default=[selected_year] if selected_year in available_years_seasonal else available_years_seasonal[-1:],
                help="Välj ett eller flera år för att analysera säsongsmönster"
            )
        
        with col2:
            if len(available_years_seasonal) > 1:
                st.info(f"📊 {len(available_years_seasonal)} år tillgängliga")
            else:
                st.warning("⚠️ Endast 1 års data")
        
        if selected_years_seasonal:
            ...
            with col2:
                show_bands = st.checkbox(
                    ...
                    value=len(selected_years_seasonal) > 1,
                    disabled=len(selected_years_seasonal) <= 1,
            ...
            with st.spinner("🔄 Hämtar säsongsdata..."):
                seasonal_data_df = get_seasonal_data(selected_company_id, selected_years_seasonal)
            
            if not seasonal_data_df.empty:
                # Beräkna säsongsmätvärden
                seasonal_metrics_df = calculate_seasonal_metrics(seasonal_data_df, selected_accounts, selected_years_seasonal)
                ...
                if not seasonal_metrics_df.empty:
                    ...
                    if value_display == "Absolut (tkr)":
                        ...
                        # Skapa andel-diagram
                        st.markdown("#### 📊 Andel av årsintäkt per månad")
                    ...
                    if show_bands and len(selected_years_seasonal) > 1:
                    ...
                    st.download_button(
                        ...
                        data=csv_data,
                        file_name=f"säsongsanalys_{selected_company_name}_{'-'.join(map(str, selected_years_seasonal))}.csv",
                        mime="text/csv"
                    )
